...
# Florida voter race tagging assessment - CVAP data
...
##    computed for the 2022 Florida voter file 
...
names(vf)[names(vf) == "Residence_Addresses_CensusBlockGroup"] <- 'block'
names(vf)[names(vf) == "Residence_Addresses_CensusTract"] <- 'tract'
names(vf)[names(vf) == "Voters_FIPS"] <- 'county'
...
taggedvf <- taggedvf %>%
  mutate(
    ethnic_desc = toupper(CountyEthnic_Description),     
...
# plot all 5 curves
plot(roc_white, col = "blue",  lwd = 2, main = "ROC Curves for GA BISG Race Prediction")
